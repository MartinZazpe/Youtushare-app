...
@WebServlet("/VerCancionesUsuario")
public class VerCancionesUsuario extends HttpServlet implements DAO_Constantes{
	private static final long serialVersionUID = 1L;
       

    public VerCancionesUsuario() {
        super();
	   ...
	   //Check that the user in session still exists in db.
	    if((Integer)session.getAttribute("usuario_id") != null) {	    	
	    	...
	    	if((Integer)session.getAttribute("usuario_id") == (buscarUsuarioPorId.getId())){
	    		...
	    		//si el usuario existe ejecutar el metodo normalmente.
	    	...
	    	//si el usuario en session ya no existe en la db:
				...
				session.removeAttribute("usuario_rol");
	  	   	    ruta = VISTA_INICIO;
	    		System.out.println("user found but mismatch with session, FIX REQUIRED!");
	    	}
	    }
	    else {
